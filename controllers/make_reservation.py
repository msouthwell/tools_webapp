...
def make_reservation():
	...
	tool = request.forms.get("tool", '')
	reserved_tools = eval(request.forms.get('reserved_tools', '[]'))
	...
	try:
		...
		print ('Tool: ' + str(tools[0]))
				
		if request.forms.get('Add Tool', '').strip():
			reserved_tools.append(tools[0])

	except pymysql.err.Error as e:
		return template('error.tpl', message='An error occurred. Error {!r}, errno is {}'.format(e, e.args[0]))

	return {'start_date':start_date, 'end_date':end_date, 'categories':categories, 'tools':tools, 'selected_category':category, 'reserved_tools':json.dumps(reserved_tools, default=decimal_default), 'message':''}
